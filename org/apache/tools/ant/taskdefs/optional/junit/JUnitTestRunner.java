...
public class JUnitTestRunner implements TestListener, JUnitTaskMirror.JUnitTestRunnerMirror {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            else if(args[i].startsWith("junitCoverDir")){
                System.getProperties().put("junitCoverDir", args[i].substring(args[i].indexOf("=") + 1));
            }
            else if(args[i].startsWith("jacocoPort")){
                System.getProperties().put("jacocoPort", args[i].substring(args[i].indexOf("=") + 1));
            }
            else if(args[i].startsWith("jacocoAddress")){
                System.getProperties().put("jacocoAddress", args[i].substring(args[i].indexOf("=") + 1));
            }
        }
